...
class _ChatScreenState extends State<ChatScreen> {
  ...
  Future<void> _handleMessage() async {
    ...
    try {
      ...
      final response =
          await _geminiService.getChatResponse(userMessage, chatHistory);

      if (kDebugMode) {
        print(
            'ChatScreen: Received response from Gemini: ${response.substring(0, min(50, response.length))}...');
        print(
            'ChatScreen: typingIndicatorIndex: $typingIndicatorIndex, messages length: ${_messages.length}');
      }
      ...
      if (mounted) {
        ...
        setState(() {
          ...
          if (typingIndicatorIndex >= 0 &&
            ...
            if (kDebugMode) {
              print(
                  'ChatScreen: Removing typing indicator at index $typingIndicatorIndex');
            }

            // Remove the typing indicator and spacing
            _messages
                .removeAt(typingIndicatorIndex + 1); // Remove height SizedBox
            _messages.removeAt(typingIndicatorIndex); // Remove typing message

            if (kDebugMode) {
              print(
                  'ChatScreen: Adding bot response at index $typingIndicatorIndex');
            }
            ...
            _messages.insert(
                typingIndicatorIndex + 1, const SizedBox(height: 20));
          } else {
            if (kDebugMode) {
              print(
                  'ChatScreen: Typing indicator not found, adding response at end');
